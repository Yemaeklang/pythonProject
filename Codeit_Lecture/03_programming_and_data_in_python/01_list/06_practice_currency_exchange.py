...
# 원화(￦)에서 달러($)로 변환하는 함수
def krw_to_usd(krw):
    return krw / 1000  # usd : krw = 1 : 1000


# 달러($)에서 엔화(￥)로 변환하는 함수
def usd_to_jpy(usd):
    return 1000 / 8 * usd  # jpy : usd = 1000 : 8
...
sequence = 0
while sequence < len(prices):
    prices[sequence] = krw_to_usd(prices[sequence])
    sequence += 1
...
sequence = 0
while sequence < len(prices):
    prices[sequence] = usd_to_jpy(prices[sequence])
    sequence += 1
